...
CLASS ltc_awsex_cl_iam_actions IMPLEMENTATION.
  ...
  METHOD get_policy_version.
    ...
    DATA lv_default_version_id TYPE /aws1/iampolicyversionidtype.

    " Use test policy created in class_setup
    " First get the default version ID
    ...
    LOOP AT lt_versions INTO DATA(lo_version_meta).
      IF lo_version_meta->get_isdefaultversion( ) = abap_true.
        lv_default_version_id = lo_version_meta->get_versionid( ).
        EXIT.
      ENDIF.
    ENDLOOP.

    cl_abap_unit_assert=>assert_not_initial(
      act = lv_default_version_id
      msg = |Should find a default version ID| ).

    " Get the specific policy version
    ao_iam_actions->get_policy_version(
      EXPORTING
        iv_policy_arn = av_test_policy_arn
        iv_version_id = lv_default_version_id
      IMPORTING
        oo_result = lo_result ).
    ...
    cl_abap_unit_assert=>assert_bound(
      ...
      msg = |Policy version object should not be initial| ).

    cl_abap_unit_assert=>assert_equals(
      exp = lv_default_version_id
    ...
    cl_abap_unit_assert=>assert_true(
      act = lo_policy_version->get_isdefaultversion( )
      msg = |Version should be marked as default| ).
  ENDMETHOD.
