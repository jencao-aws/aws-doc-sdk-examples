...
CLASS ltc_awsex_cl_ec2_actions IMPLEMENTATION.
  ...
  METHOD stop_and_start_instances.
    ...
    TRY.
      ...
      CATCH /aws1/cx_ec2incorrectinstst INTO DATA(lo_state_ex).
        " Expected if instance is still stopping - this is OK for SDK testing
  ...
  ENDMETHOD.
    ...
    " Note: Uses a fresh instance to ensure clean state
    DATA(lv_uuid) = /awsex/cl_utils=>get_random_string( ).
    ...
    " Wait for instance to be running (with timeout management)
    DATA(lv_status) = wait_for_instance( iv_instance_id = lv_instance_id iv_required_status = 'running' ).
    IF lv_status <> 'running'.
    ...
    " Use DescribeInstanceStatus for more efficient state checking
