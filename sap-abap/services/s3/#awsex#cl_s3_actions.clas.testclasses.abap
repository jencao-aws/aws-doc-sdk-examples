...
CLASS ltc_awsex_cl_s3_actions IMPLEMENTATION.
  ...
  METHOD list_buckets.
    ...
    " Verify at least our test buckets exist
    DATA(lt_buckets) = lo_result->get_buckets( ).
    cl_abap_unit_assert=>assert_not_initial(
      act = lt_buckets
      msg = |No buckets returned| ).

    " Check that at least one of our test buckets is in the list
    DATA(lv_found) = abap_false.
